...
function fetchIssues(topicIndex,repoIndex,err,res,html){
    repoCounts++;
    const $=cheerio.load(html);
    let issueAnchotTags=$(".Link--primary.v-align-middle.no-underline.h4.js-navigation-open.markdown-title");
    for(let i=0;i<8 && i<issueAnchotTags.length;i++){
        gitTopics[topicIndex].repos[repoIndex].issues.push({
            issueName:$(issueAnchotTags[i]).text(),
            issueUrl:"https://www.github.com" + $(issueAnchotTags[i]).attr("href")
        })
    }
